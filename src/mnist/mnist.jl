...
function create_mnist_sa_problem(n_samples::Int=1000; device=cpu, outputs=10, dataset_seed=32422, model_seed=76234, kwargs...)
    data, labels, labels_one_hot = generate_mnist_dataset(n_samples; device, outputs, seed=dataset_seed, kwargs...)
    model = generate_mnist_model(; device, outputs, seed=model_seed, kwargs...)
...
function solve_mnist_trajectory(; τ=4, s=50.0, σ=0.001, epochs=10000, n_samples=2048, device=cpu, outputs=2, show_progress=false, fraction_to_include=1.0, max_perturb_models::Int=nothing, kwargs...)
    problem, info = create_mnist_trajectory_state_and_loss(τ, σ, n_samples; device, outputs)
    alg = MetropolisHastings.get_shooting_and_bridging_mh_alg(s, σ; fraction_to_include=fraction_to_include, max_width=max_perturb_models)
    iter = show_progress ? ProgressBar(1:epochs) : epochs
    solution = solve(problem, alg, iter)
    info[:solution] = solution
    info[:s] = s
